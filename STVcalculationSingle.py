...
def GetFile():
    ...
    try:
        ...
        pymsgbox.confirm(text='You did not read a CTG data file. Make a choice.', buttons=['Try again','Quit'])
        if pymsgbox.__replyButtonText == 'Try again':
            GetFile()
        else:
            raise SystemExit
...
#read data from binary MOSOS file with .ctg (new version)
    ...
    mCount1 = 0
    mCount2 = 0
# To detect if channel 1 or 2 was connected to the US probe (should always be 1 in singletons, but sometimes a mistake is made)    
    while mTel < len(mData) and mCount1 < 100 and mCount2<100:
        mB = bin(mData[mTel])
    ...
    if mCount1 >= 100:
        ...
        pymsgbox.confirm('File contains insufficient data. Make a choice.', buttons=['New file', 'Quit'])
        if pymsgbox.__replyButtonText == 'New file':
            GetFile()
        else:
            raise SystemExit
    mTel -= 12
...
# Setting the X-axis values        
    ...
    while mTel < len(mFHR):
        ...
        mTel += 1
# Calculation of interval from FHR and exckusuon of extremes <30/mi or > 200/min
    mTel = 0
    while mTel < len(mFHR):
        if mFHR[mTel] > 0:
    ...
    if np.count_nonzero(mFHR)< 2400:
        pymsgbox.alert('File contains less than 10 minutes CTG data. Select another file.')
        GetFile()
...
#If the file contains only FHR then these data will be read and uterine pressure remains 0
    ...
    f.close()
    if np.count_nonzero(mFHR)< 2400:
        pymsgbox.confirm('File contains less than 10 minutes CTG data. ', buttons=['New file','Quit'])
        if pymsgbox.__replyButtonText == 'New file':
            GetFile()
        else:
            raise SystemExit
...
def CalcSTV():
    ...
    global mReport, mExport, mFileName, mReportFileName
    global mSTVmin, mLTVlow, mLTVhigh
    CalcBaseline()
    mSTVmin = []
    mSTVX = []
    mAccelLoc = []
...
# dit is code 192 (decelleratie), en 185 en 200 (uitbijters)
    ...
    while mTel < len(mFHRcode):
        if mFHRcode[mTel] == 187 or mFHRcode[mTel] == 190 or mFHRcode[mTel] == 195 or mFHRcode[mTel] == 197:
            mFHRcode[mTel] = 1
        mTel += 1
...
# Calculate low LTV minutes
    ...
    i = 0
    j = 0
    while mTel < len(mLTVlow):
        if mLTVhigh[mTel] == 175 and mLTVlow[mTel] == 170:
            mLTVhigh[mTel] = 0
            j += 1
        elif mLTVhigh[mTel] == 175:
            i += 1
        elif mLTVlow[mTel] == 170:
            j += 1
        if mTel > 0 and mTel < 60:
            mHighLTVtime60 = round(100 * i / mTel)
            mLowLTVtime60 = round(100 * j / mTel)
        mTel += 1
    mHighLTVtime = round(100 * i / mDuration)
    mLowLTVtime = round(100 * j / mDuration)
...
# Berekenen ratio STV/LTV en sinus patroon
    mSLratio = mSTV/mLTV
    mSLZ = (mSLratio - 0.186) / 0.024
    mSinRep = ''
    if mSLZ > 1 and mSTV > 4:
        mTel = 0
        i = 0
        mFHRepS = []
        mFHRepX = []
        ...
        while mTel < len(mFHRep):
            if mFHRcode[mTel] == 1:
                mFHRepS.append(mFHRep[mTel])
                mFHRepX.append(i)
                i += 1
            mTel += 1
        if i >= 320:
            mTel = 0
            while mTel < len(mFHRepS):
                mFHRepTopInt.append(0)
                mFHRepDipInt.append(0)
                mTel += 1
            mTel = 0
            i = 0
            j = 0
            mStart = 0
            while mTel < len(mFHRepS)-2 and mStart == 0:
                if mFHRepS[mTel] > 0 and (mFHRepS[mTel] < mFHRepS[mTel+1] or mFHRepS[mTel] < mFHRepS[mTel+2]):
                    mStart = 1
                    mTel -= 1
                elif (mFHRepS[mTel] > mFHRepS[mTel+1] and mFHRepS[mTel] > mFHRepS[mTel+2] 
                and mFHRepS[mTel+1]>0 and mFHRepS[mTel+2] > 0):
                    mStart = 2
                    mTel -= 1
                mTel += 1
            while mTel < len(mFHRepS) - 2:
                if (mFHRepS[mTel] > mFHRepS[mTel+1] and mFHRepS[mTel] > mFHRepS[mTel+2] 
                and mFHRepS[mTel+1]>0 and mFHRepS[mTel+2] > 0 and mStart ==1):
                    mFHRepTopInt[mTel] = i
                    mStart = 2
                    i = 0
                if mFHRepS[mTel] < mFHRepS[mTel+1] and mFHRepS[mTel] < mFHRepS[mTel+2] and mFHRepS[mTel]>0 and mStart == 2:
                    mFHRepDipInt[mTel] = j
                    mStart = 1
                    j = 0                
                else:
            ...
            End1 = 0
            End2 = 0
            while mTel < len(mFHRepTopInt):
                if mFHRepTopInt[mTel] > 0 and End1 == 0:
                    End1 = mFHRepS[mTel]
                elif mFHRepDipInt[mTel] > 0 and End2 == 0:
                    End2 = mFHRepS[mTel]
                ...
                mTel += 1
            mAmpAv = mAmp / i
            mDist = np.bincount(mFHRepTopInt)
            mFreqT = np.argmax(mDist[3:9]) + 3
            mFreqTprop = np.sum(mDist[mFreqT-1:mFreqT+2]) / np.sum(mDist[1:])
            mDist = np.bincount(mFHRepDipInt)
            mFreqD = np.argmax(mDist[3:9]) + 3
            mFreqDprop = np.sum(mDist[mFreqD-1:mFreqD+2]) / np.sum(mDist[1:])
            mFreq = 60/ (((mFreqT + mFreqD) / 2) * 3.75)
            mFreqProp = (mFreqTprop + mFreqDprop) / 2
            if mFreqProp > 0.65:
                mSinRep = (', **** Possible HF sinusoid pattern, freq.' + str(round(mFreq,1)) + 
                       '/min, Amp. ' + str(round(mAmpAv,1)) + 'msec ' )
        else:
            mSinRep = ', **** STV / LTV ratio > 1, but not enough signal length to exclude HF sinus pattern. '
    elif mSLZ < -2:
        mSinRep = ', **** Possible baseline error or LF sinusoid pattern. '
# Rapportage
    ...
    mReport = ('Base FHR : ' + str(round(mFHRmean)) + ', STV : ' + str(round(mSTV, 2))
               ...
               '\n STV/LTV Z-score: ' + str(round(mSLZ,2)) + mSinRep  + ', Duration CTG : ' + str(mDuration) + ' minutes, Valid minutes : ' + str(mValid) +
...
## Create plotwindow
    ...
    global mFHRep, mFHRXep, mFHRbase, mFHRcode, mAccelLoc, mDecelLoc, mLTVlow, mLTVhigh, mUtEp
    global mUtP, mXdata
    global Plot1, Plot2, pl1, pl2, pl3, win
# Last calculations
    mPlotTitle = mFileName
    mTel = 0
    while mTel < len(mFHRcode):
...
#    pl1.plot(mXdata, mFHRN, connect="finite", pen='k', color='b')
    ...
    if np.sum(mLTVlow) > 0:
        pl1.plot(range(0, len(mLTVlow)), mLTVlow, pen=None, symbol ='t', symbolPen=(0,128,0), symbolSize=12)
    if np.sum(mLTVhigh) > 0:
        pl1.plot(range(0, len(mLTVhigh)), mLTVhigh, pen=None, symbol ='t1', symbolPen=(0,128,0), symbolSize=12)        
    Lbl1.setText(mReport)
...
vb = CustomViewBox()
Btn1 = QtGui.QPushButton(text='  Get new file  ')
Btn1.clicked.connect(GetFile)
...
Lbl1.setText("Report")
Lbl1.setFont(QtGui.QFont('SansSerif', 13))
