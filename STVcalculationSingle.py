...
# Calculation starting point
    ...
    if mStartHRe == 0:
        mStartHRe = mRefHRe
        mStartTe = mRefTe
    print(mFileName, mRefHRe, mStartHRe)
...
# Set invalid epochs to mStartHre
    while mTel < len(mIntEp):
        if mTel >= mLapseT:
            mMeanIntRec = np.mean(mTbase[mTel-mLapseT:mTel-mLapseT_B])
        if (mTel < mLapseT and (mIntEp[mTel] == 0 or mIntEp[mTel] < (mStartTe - mLimit) or 
                               mIntEp[mTel] > (mStartTe + mLimit))):
...
#        mTel += 1
    ...
    plot.clear()
    pl2.clear()
    plot.showGrid(x=True, y=False, alpha=100)
    plot.setXRange(0, 60)
    plot.setBackground('#e6e6e6')
    plot.setTitle(' CTG file  ' + mFileName)
